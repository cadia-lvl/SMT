...
if ((FIRST_STEP <= 6 && LAST_STEP >= 6)); then
  ...
  for test_set in "${test_sets[@]}"; do
    ...
    preprocessing/main.py tokenize - "$TEST_SET_IN_PROCESSED" "$LANG_FROM" --tokenizer bpe
    ...
    /opt/moses/bin/moses -f "$BINARISED_DIR"moses.ini \
      ...
      >"$TEST_SET_TRANSLATED"
    # Postprocess
    preprocessing/main.py detokenize "$TEST_SET_TRANSLATED_POSTPROCESSED" - "$LANG_TO" --tokenizer bpe
