...
# Data
TRAINING_DATA="$DATA_DIR"train/form/data-uni-train-30
DEV_DATA="$DATA_DIR"dev/form/data-uni-train-30
TEST_DIR="$DATA_DIR"test/raw/

LM="$LM_SURFACE_5"-uni-train-30."$LANG_TO"
LM_ORDER=5
...
if ((FIRST_STEP <= 6 && LAST_STEP >= 6)); then
  ...
  for test_set in "${test_sets[@]}"; do
    ...
    preprocessing/main.py tokenize "$TEST_SET_IN"-tmp "$TEST_SET_IN_PROCESSED" "$LANG_FROM" --tokenizer bpe --model preprocessing/preprocessing/resources/"$LANG_FROM"-uni-train-30.model
    ...
    /opt/moses/bin/moses -f "$BINARISED_DIR"moses.ini \
      ...
      >"$TEST_SET_TRANSLATED"
    # Postprocess
    preprocessing/main.py detokenize "$TEST_SET_TRANSLATED" "$TEST_SET_IN"-tmp "$LANG_TO" --tokenizer bpe --model preprocessing/preprocessing/resources/"$LANG_TO"-uni-train-30.model
