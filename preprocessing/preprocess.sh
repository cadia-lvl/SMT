...
# Enrich
# python "$DIR"/main.py enrich "$WORK_DIR"/received.pickle "$WORK_DIR"/enriched.pickle --chunksize 4000

# Split
# python "$DIR"/main.py split "$WORK_DIR"/enriched.pickle "$WORK_DIR"/train.pickle "$WORK_DIR"/dev.pickle
...
for set in "train dev test-ees test-ema test-opensubtitles"; do
    python "$DIR"/main.py truecase "$WORK_DIR"/"$set".pickle --lang en --segment form "$TRUECASE_FORM_EN" "$WORK_DIR"/"$set"-true.pickle
    python "$DIR"/main.py truecase "$WORK_DIR"/"$set"-true.pickle --lang is --segment form "$TRUECASE_FORM_IS" "$WORK_DIR"/"$set"-true.pickle
    python "$DIR"/main.py truecase "$WORK_DIR"/"$set"-true.pickle --lang en --segment lemma "$TRUECASE_LEMMA_EN" "$WORK_DIR"/"$set"-true.pickle
    python "$DIR"/main.py truecase "$WORK_DIR"/"$set"-true.pickle --lang is --segment lemma "$TRUECASE_LEMMA_IS" "$WORK_DIR"/"$set"-true.pickle
done
